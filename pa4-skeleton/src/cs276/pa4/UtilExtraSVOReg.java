...
//    }
    ...
    try {
      Map<Query,List<Document>> data_map = UtilExtraSVOReg.loadTrainData (train_data_file);
      Map<String, Map<String, Double>> labelMap = null;
      if (isTrain(refFile)){
        labelMap = UtilExtraSVOReg.loadRelData(refFile);
      }
...
//    Standardize filter = new Standardize();
  ...
  public static Map<String,Map<String, Double>> getDocTermFreqs(Document d, Query q)
  {
    ...
    //initialize tfs
    for (String type : UtilExtraSVOReg.TFTYPES)
