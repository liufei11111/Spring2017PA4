...
public class GoogleWordVector {
  ...
  public static double[] getQueryDocEmbeddingSim(Map<String,Double> rawQueryFreq, Map<String,Map<String, Double>> rawDocuFreq){
    ...
    if (Config.isRazarEnabledPart4TitleSim){
      double[] simScores = new double[4];
      simScores[0]=rawDocuFreq.get("url").size()>0?getSimScore(rawQueryFreq,rawDocuFreq.get("url"),wv):0.0;//title, header, anchor, body
      simScores[1]=rawDocuFreq.get("header").size()>0?getSimScore(rawQueryFreq,rawDocuFreq.get("header"),wv):0.0;
      simScores[2]=rawDocuFreq.get("anchor").size()>0?getSimScore(rawQueryFreq,rawDocuFreq.get("anchor"),wv):0.0;
      simScores[3]=rawDocuFreq.get("body").size()>0?getSimScore(rawQueryFreq,rawDocuFreq.get("body"),wv):0.0;
      return simScores;
    }
